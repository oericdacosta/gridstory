...
def find_optimal_k(
    ...
    if k_range is None:
        config = get_config()
        k_min = config.get_k_range_min()
        k_max = config.get_k_range_max()
...
def cluster_laps_dbscan(
    ...
    Args:
        ...
        eps: Distância máxima entre voltas para serem consideradas vizinhas.
            Se None, estima automaticamente usando média das distâncias ou carrega de config.yaml
        min_samples: Mínimo de voltas para formar um ritmo consistente (padrão: carrega de config.yaml)
    ...
    if min_samples is None:
        config = get_config()
        min_samples = config.get_dbscan_min_samples()
    if eps is None:
        config = get_config()
        eps_config = config.get_dbscan_eps()
    else:
        eps_config = eps
    ...
    if group_by:
        ...
        for group_name, group_df in df.groupby(group_by):
            ...
            eps_value = eps_config
            if eps is None and eps_value is None:
